...
| :--: | :--: | ------------------------------------------------------------ | :----------: | :------: | :------: |
|  68  | 功能 | 可以给软件安装的用户设置别名，便于识别。                     |              |  0.2.8   |          |
|  67  | 功能 | 标记筛选添加可多选功能。                                     |              |  0.2.8   |          |
|  66  | 功能 | 需要一个针对网络可能失败的安全传输（可加开关）。             |              |  0.2.8   |          |
|  65  | 功能 | 添加一个额外的Http组件用于简单传输文件。                     |              |  0.2.7   |  0.2.8   |
|  64  | 功能 | 添加多个主题设置选择。                                       |              |  0.2.7   |  0.2.8   |
|  63  | 优化 | 取消任务时的通知消息异步发送，防止卡顿。                     |              |  0.2.7   |  0.2.8   |
|  62  | 优化 | 优化对照界面配置文件结构，添加Mark和Type字段进行功能辅助。   |              |  0.2.7   |  0.2.8   |
|  61  | 功能 | 文件浏览页面支持打开本地文件夹（远程不支持）。               |              |  0.2.7   |  0.2.8   |
|  60  | 问题 | 对照尝试访问路径偶发不生效。                                 |              |  0.2.7   |  0.2.8   |
|  59  | 功能 | 支持压缩，解压缩文件（ZIP格式）。                            |              |  0.2.7   |  0.2.8   |
|  58  | 功能 | 添加SHA256校验。                                             |              |  0.2.6   |  0.2.7   |
|  57  | 问题 | 引申自问题55，有数据发送失败应当终止任务。                   |              |  0.2.7   |          |
|  56  | 问题 | 使用时frelayConsole在Linux上路径拼接有问题。                 |              |  0.2.7   |  0.2.7   |
|  55  | 问题 | 第三个客户端在A，B传输文件时，参与通信，会导致比如A有一帧数据发送超时。 |              |  0.2.7   |  0.2.7   |
|  54  | 功能 | 传输涉及文件夹时需要询问确认。                               |              |  0.2.7   |  0.2.7   |
|  53  | 功能 | 客户端可以添加锁定IP功能，锁定删除功能、单向功能全部锁定禁用。 | 目前禁用部分 |  0.2.6   |  0.2.7   |
|  52  | 问题 | 发送大文件时，UI有不流畅现象。                               |              |  0.2.6   |  0.2.7   |
...
|  44  | 问题 | 出现过多选下载显示远程文件不存在问题，但是单个下没问题。     |     偶发     |  0.2.4   |  0.2.7   |
...
|  26  | 功能 | 最好能保存关闭界面时Splitter和UI的尺寸。                     |              |  0.2.2   |    x     |
|  25  | 功能 | 手动输入的文件夹访问路径保存历史记录，以便后续可能再次使用（10条）。 |              |  0.2.2   |  0.2.6   |
|  24  | 问题 | 发送端如果发送的数据比较大，UI会卡住（功能是正常的）。       |              |  0.2.2   |  0.2.3   |
|  23  | 问题 | 作为接收端如果对方断联要处理这个情况。                       |              |  0.2.2   |  0.2.3   |
|  22  | 功能 | 配置可以切组。                                               |              |  0.2.2   |  0.2.8   |
|  21  | 功能 | 可以传输文件夹。                                             |              |  0.2.2   |  0.2.7   |
