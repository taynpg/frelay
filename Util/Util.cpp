...
QStringList Util::ToList(const QVector<QString>& vec)
{
    QStringList list;
    for (const auto& item : TAS_CONST(vec)) {
        list.push_back(item);
    }
    return list;
}

QVector<QString> Util::ToVec(const QStringList& list)
{
    QVector<QString> vec;
    for (const auto& item : TAS_CONST(list)) {
        vec.push_back(item);
    }
    return vec;
}
