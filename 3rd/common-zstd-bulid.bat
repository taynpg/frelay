...
set "SCRIPT_DIR=%~dp0"
set "SOURCE_DIR=%SCRIPT_DIR%zstd-1.5.7"
set "BUILD_BASE=%SCRIPT_DIR%zstd-1.5.7"
...
set "INSTALL_DIR=%SCRIPT_DIR%binary\common\zstd"
...
set "DEBUG_BUILD_DIR=%BUILD_BASE%\debug-build"
...
cmake -B"%DEBUG_BUILD_DIR%" -S"%SOURCE_DIR%\build\cmake" ^
...
set "RELEASE_BUILD_DIR=%BUILD_BASE%\release-build"
...
cmake -B"%RELEASE_BUILD_DIR%" -S"%SOURCE_DIR%\build\cmake" ^
    -DZSTD_BUILD_SHARED=OFF ^
    -DZSTD_BUILD_STATIC=ON ^
    -DZSTD_BUILD_PROGRAMS=OFF ^
