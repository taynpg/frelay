# frelay

一个通过中继服务器在多个客户端之间相互传输文件的工具。

## 功能特点

1. 极简配置——只需启动服务器，连接至服务器，即可开始传输文件。
2. 若需频繁传输相同文件，frelay 提供本地-远程文件映射功能，支持一键传输。

## 组件

项目构建有三个二进制结果：

| 名称          | 类型   | 功能                                                         |
| ------------- | ------ | ------------------------------------------------------------ |
| frelayServer  | 控制台 | 中继服务器。                                                 |
| frelayConsole | 控制台 | 命令行客户端，没有操作功能，一般配合另外一个带GUI的客户端使用。 |
| frelayGUI     | GUI    | GUI客户端。                                                  |

## 示例图

![示例图片](img/10.png)

![示例图片](img/xp.png)

## 注意事项

- 请使用版本一致的三个组件，不可混用。
- 终端（例如低版本Win的cmd）如果不支持UTF-8输出，则frelayConsole中文显示部分会乱码，但是功能是正常的。

## 免责声明与警告

本工具仅设计用于通过中继服务器在客户端之间进行简单的文件传输，未包含任何可防范数据攻击或类似威胁的安全验证逻辑，主要适用于临时性、非敏感的文件传输场景。请在不使用时及时关闭软件或断开与中继服务器的连接。

本软件可能存在意外错误，可能导致文件损坏或丢失。请务必做好文件备份，或仅将其用于非关键文件的传输。

# 停止维护

截止到`0.2.8`版本`frelay`基本传输功能（包括文件夹传输、新建、删除、更名、SHA256校验，ZIP解压缩等）均可正常使用。

且已满足XP下的基本传输需求，鉴于为了兼容XP使用过低的编译器版本，导致无法使用新标准语法以及使用三方库的不便性，该仓停止维护。

将会启用新的仓库[FileTransfer](https://github.com/taynpg/FileTransfer)，新仓有如下主要变动：
- 使用更新的编译器(C++20支持)，以便更方便的使用对应编译器的库和新C++语法。
- 通信核心部分使用`asio`替代`QNetwork`，核心与GUI只通过接口交互，分层隔离。
- 更合理的传输逻辑，任务逻辑。
- 更安全的传输。
- 更快的传输。
- 更多的压缩格式支持。
